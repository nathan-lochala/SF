...
@section('content')
    ...
    @include('_content.title',['heading' => 'Add Members <small>' . $group->name . '</small>','body' => 'Add one or more members to the ' . $group->name . ' Study Group.'])

    <!----------------------------------------------------------------------------->
    <!----------------------------------------------------------------------------->
    @include('_forms.new-head',['color' => 'panel-info'])
            <!--NEW FORM-->
    {!! Form::open(['id' => 'admin-form','url' => 'study_group/' . $group->id . '/add_members']) !!}
